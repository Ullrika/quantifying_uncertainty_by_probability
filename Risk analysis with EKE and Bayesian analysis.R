...
#############################################################################
## Analyse risk after learning about the parameters from data using priors 
## informed by expert knowledge elicitation
#############################################################################
{
...
data_to_model = list(shape = ej$Gamma$shape, rate = ej$Gamma$rate, mean_mean_speed = 1.4, 
                     obs_waiting_time = obs_waiting_time, obs_speed = obs_speed, 
                     n_obs_waiting_time = length(obs_waiting_time), n_obs_speed = length(obs_speed),
                     N = N, distance = distance, leave_earlier = leave_earlier, 
                     n_alt = length(leave_earlier))
...
## Sample from the posterior
posterior_sample = coda.samples(Bayesian_sampler, c('mean_waiting_time','mean_speed'), 
                                n.iter=round(N/3), thin=1)
## Check convergence
plot(posterior_sample)
sim = as.data.frame(as.matrix(posterior_sample)) ## This is created to make it easier to execute some commands below
...
predictive_sample = coda.samples(Bayesian_sampler, c('rel_freq'), n.iter=round(N), thin=1)
